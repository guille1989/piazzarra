...
class inventarioPopayanCentro extends Component {
    constructor(props) {
        super(props);
        this.state = {
            inventario: [],
    ...
    componentDidMount(){
        ...
        fetch(`http://${process.env.REACT_APP_URL_PRODUCCION}/api/revisioninventariofecha/` + today + `/Pizzarra-Popayan-Centro`, requestOptions)
        .then(response => response.json())
        .then(data => {
    ...
    handleFechaHoy(e){
        ...
        const requestOptions ={
          ...
          fetch(`http://${process.env.REACT_APP_URL_PRODUCCION}/api/revisioninventariofecha/` + e.target.value + `/Pizzarra-Popayan-Centro`, requestOptions)
              .then(response => response.json())
              .then(data => {
                if(data.inv.length === 0){
                    //console.log("No hay")
    ...
    handleIngresarInventario(){
        if(window.confirm("Seguro desea enviar inventario ??")){
            ...
            let INVENTARIO_ID = 'Pizzarra-Popayan-Centro'
...
export default inventarioPopayanCentro;