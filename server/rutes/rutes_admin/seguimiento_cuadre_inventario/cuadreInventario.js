...
async function cuadreInventario(
  fechaInventario,
  fechaInventarioAyer,
  inv_id,
  pedidos_id
  ...
  result = await InventarioActual.find({
    $and: [
      { FECHA_INVENTARIO_ACTUAL: fechaInventario },
      { "INVENTARIO_AUX.INVENTARIO_ID": inv_id },
    ],
  });
  result_ayer = await InventarioActual.find({
    $and: [
      { FECHA_INVENTARIO_ACTUAL: fechaInventarioAyer },
      { "INVENTARIO_AUX.INVENTARIO_ID": inv_id },
    ],
  });
  result_entradas = await InventarioEntradas.find({
    $and: [
      { FECHA_INVENTARIO_ENTRANTE: fechaInventario },
      { "INVENTARIO_AUX.INVENTARIO_ID": inv_id },
    ],
  });
  ...
  if (result.length === 0) {
    result = [inv_ceros_aux];
  } else {
    result = result[0].INVENTARIO_AUX;
  }

  if (result_ayer.length === 0) {
    result_ayer = [inv_ceros_aux];
  } else {
    result_ayer = result_ayer[0].INVENTARIO_AUX;
  }

  if (result_entradas.length === 0) {
    result_entradas = [inv_ceros_aux];
  } else {
  ...
  result_ventas_aux = await PedidoPizzarra.find({
    $and: [
      { "aux.fecha_pedido": fechaInventario },
      { "aux.local": pedidos_id },
    ],
  });
  ...
  result_insumos.map((item, index) => {
    ...
    if (
      parseInt(result[0][item.TIPO]) -
        parseInt(result_ayer[0][item.TIPO]) -
        parseInt(invEntradaAux) -
        parseInt(result_ventas[item.TIPO]) <
      0
    ) {
      inv_estado = "Faltante";
    }
...
function resumenVentas(result_ventas_auxn, insumos) {
  let result = [];
