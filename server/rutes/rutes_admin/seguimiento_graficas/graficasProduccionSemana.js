...
async function leerVentasSemana(fecha_uno_aux, filtro_tipo, pedidos_aux) {
  ...
  let resutl_produccion = [];
  let resutl_produccion_aux = [];

  let result_insumos_ceros = [];

  let result_insumos = await Insumos.find({});

  result_insumos.map((item, index) => {
    let item_aux = item.TIPO;
    result_insumos_ceros = { ...result_insumos_ceros, [item_aux]: 0 };
  });

  if (filtro_tipo === "dia") {
    result = await PedidoPizzarra.find({
      $and: [
        { "aux.fecha_pedido": date_semana_search[0] },
        { "aux.local": pedidos_aux },
      ],
    });

    resutl_produccion = resumenVentas(result, result_insumos, "dia");

    result_insumos.map((item, index) => {
      let item_aux = item.TIPO;
      //console.log(resutl_produccion[item_aux])
      resutl_produccion_aux.push({
        x: [item_aux],
        y: resutl_produccion[item_aux],
      });
    });
    ...
    for (let i = 0; i < date_semana_search.length; i++) {
      ...
      result = await PedidoPizzarra.find({
        $and: [
          { "aux.fecha_pedido": date_semana_search[i] },
          { "aux.local": pedidos_aux },
        ],
      });

      resutl_produccion = resumenVentas(result, result_insumos_ceros, "semana");
    }
    ...
    for (let i = 0; i < date_semana_search.length; i++) {
      ...
      resutl_produccion = resumenVentas(result, result_insumos_ceros, "mes");
    }
    ...
    //console.log(resutl_produccion_aux)
  }

  return { resutl_produccion_aux };
}

function resumenVentas(result_ventas_auxn, insumos, periodo) {
  let result = [];
  ...
  result_ventas_auxn.map((item, index) => {
    item.aux.map((item1, index) => {
      if (
        item1.tipo_pedido === "DOMICILIO" ||
        item1.tipo_pedido === "RECOGEN"
      ) {
      ...
      item.pedido.map((item, index) => {
        ...
        if (item.tipo.includes("DESAYUNO AMERICANO")) {
          ...
          let Adiciones = item.mod_sabor_desayuno
            .replace(/Adicion /g, "")
            .split("+");
        ...
        if (item.tipo.includes("RAVIOLI SALSA")) {
          ...
          let Adiciones = item.mod_sabor_ravioli
            .replace(/Adicion /g, "")
            .split("+");
        ...
        if (item.tipo.includes("PASTA TIPO")) {
          ...
          if (item.tipo.includes("NAPOLITANA")) {
            ...
            item.tipo.includes("SALSA BLANCA") &&
            item.sabor_pasta !== "CARBONARA"
          ) {
            result.CREMA_LECHE = result.CREMA_LECHE - 95;
          }
        ...
        if (item.tipo.includes("PAN UNIDAD")) {
          result.PAN_OREGANO =
            result.PAN_OREGANO - item.tipo.replace(/^\D+/g, "");
        }
          ...
          if (item.tipo.includes("LIMONADA")) {
            if (item.tipo.includes("JARRA")) {
              if (item.mod_sabor_jugo.includes("Con Azucar")) {
                result.AZUCAR =
                  result.AZUCAR - item.tipo.replace(/^\D+/g, "") * 120;
              }
              result.LIMONES =
                result.LIMONES - item.tipo.replace(/^\D+/g, "") * 240;
            } else {
              if (item.mod_sabor_jugo.includes("Con Azucar")) {
                result.AZUCAR =
                  result.AZUCAR - item.tipo.replace(/^\D+/g, "") * 50;
              }
              result.LIMONES =
                result.LIMONES - item.tipo.replace(/^\D+/g, "") * 120;
            }
          } else {
            if (item.mod_sabor_jugo.includes("Con Azucar")) {
              result.AZUCAR =
        ...
        if (item.tipo.includes("SALSA 16 ONZAS")) {
          ...
          result.SALSA_NAPOLITANA_GALON =
            result.SALSA_NAPOLITANA_GALON - parseInt(cantidad[1]) * 472;
        }
